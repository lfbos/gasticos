...
import type { BelvoWidgetSuccessEvent, BelvoWidgetErrorEvent } from '@/types/belvo';
    ...
    belvoSDK?: {
      createWidget: (
        ...
        options: {
          ...
          onExit?: (data: { last_encountered_error?: BelvoWidgetErrorEvent }) => void;
          onEvent?: (event: { eventName: string; meta_data?: Record<string, unknown> }) => void;
        }
      ) => { build: () => void };
    };
  }
...
export function BelvoWidget({
  ...
  const handleOpenWidget = useCallback(() => {
    ...
    try {
      ...
      onError?.({ error_code: 'WIDGET_ERROR', error_message: 'Failed to open widget' });
    }
  }, [isScriptLoaded, accessToken, onError]);

  const isButtonDisabled = disabled || !isScriptLoaded || !accessToken || isLoading;
